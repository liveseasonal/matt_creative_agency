...
class Kameleon_field_slider{
	...
	function render(){
		...
		$element .= '<div class="km-slider-range-container"><input type="number" step="1" min="'.$min.'" max="'.$max.'" size="4" data-element-type="'.$elementType.'" onchange="km_cf7_onfield_change(this)"  data-cssname="'.$cssName.'" data-csssufix="'.$cssSufix.'" data-type="number" onkeyup="kameleon_change_range(this)"  value="'.esc_attr($value).'"  class="km-slider-input kmcf7_maker_inp" min="'.$min.'" max="'.$max.'" size="4">
		<div class="km-slider-range-parent"><div class="km-slider-range-filled" ></div>
		<input type="range" step="1" value="'.esc_attr($value).'" data-type="slider"  data-element-type="'.$elementType.'" class="km-slider-range kmcf7_maker_inp"  min="'.$min.'" max="'.$max.'" size="4"  data-cssname="'.$cssName.'" oninput="kameleon_change_range(this)" onchange="kameleon_change_range(this)"></div></div>';


		return kameleon_preint_field_section($title, $description, $element);			
	}
