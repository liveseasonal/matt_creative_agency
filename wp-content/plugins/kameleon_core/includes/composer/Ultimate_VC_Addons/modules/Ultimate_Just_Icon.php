...
if(!class_exists('AIO_Just_Icon'))
{
	class AIO_Just_Icon
	{
		...
		{
			$icon_type = $icon_img = $img_width = $icon = $icon_color = $icon_color_bg = $icon_size = $icon_style = $icon_border_style = $icon_border_radius = $icon_color_border = $icon_border_size = $icon_border_spacing = $icon_link = $el_class = $icon_animation =  $tooltip_disp = $tooltip_text = $icon_align = $sy_hover_style = $sy_hover_background = $sy_hover_color = '';
			...
			if($icon_type == 'custom'){
				...
				if($icon !== ""){
					$dataIcon = '';
					if($icon_style == 'advanced')
						$dataIcon = 'data-syai-effectstyle="'.$sy_hover_style.'" data-syai-haseffect="yes"';

					$output .= "\n".$link_prefix.'<div class="aio-icon '.$icon_style.' '.$elx_class.'" '.$css_trans.' style="'.$style.' --icon-hover-color:'.$sy_hover_color.'; --icon-hover-background:'.$sy_hover_background.'; " '.$dataIcon.'>';
					$output .= "\n\t".'<i class="'.$icon.'"></i>';
					$output .= "\n".'</div>'.$link_sufix;
				}
				$output = $output;
