...
function sy_postgrid_shortC($atts) {  
	...
	$title_font = sy_composer_font_styles($pg_title_font, $pg_title_size, $pg_title_style);
	$title_style = 'color:'.$pg_title_color.';'.$title_font;

	$excerpt_font = sy_composer_font_styles($pg_excerpt_font, $pg_excerpt_size, $pg_excerpt_style);
	...
	if ( have_posts() ) : 
		while ( have_posts() ) : the_post();
			...
			if($pg_style == 'style1' || $pg_style == 'style2'){
				$thumb = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()) ,'kameleon-large-thumbnail');	
				$thumbnailUrl = $thumb['0'];				
				$thumbnailContainer = '<div class="syp-itemgrid-link"><img src="'.$thumbnailUrl.'" alt="Image"/></div>';
				...
				$infoContainer = '<div class="syp-itemgrid-info" style="border-color:'.$pg_border_inside.'; color:'.$pg_excerpt_color.'; padding-left:'.$pg_paddingleftright.'px">
									<div class="syp-itemgrid-author"><i class="fa fa-user"></i>'.esc_html__('By ','kameleon').''.get_the_author().'</div>
			...
			if($pg_style == 'style3'){
				$authorContainer = '<div class="syp-itemgrid-3-bottom" style="border-top:'.$pg_border_inside_thickness.'px solid'.$pg_border_inside.'; color:'.$pg_title_color.';">
									...
									'</div>
									<span>'.esc_html__('By ','kameleon').''.get_the_author().'</span>
								</div>';
				$dateSection  = '<div class="syp-itemgrid-date3"  style="color:'.$pg_title_color.';">'.get_the_time("M jS Y").'</div>';
			}


			$outPut .= '<div class="'.$p_classes.'" style="'.$container_style.'" data-style="'.$pg_style.'">
							'.$thumbnailContainer.'								
							<div class="syp-itemgrid-center" style="'.$center_style.'">
								'.$userThumbnail.'
								'.$date_style4.'
								<div class="syp-itemgrid-title" style="'.$title_style.'">		
									<a href="'.esc_url(get_the_permalink()).'">'.get_the_title().'</a>								
								</div>
								'.$dateSection.'
								<div class="syp-itemgrid-excerpt" style="'.$excerpt_style.'">
...
add_shortcode( 'sy_postgrid', 'sy_postgrid_shortC' );
