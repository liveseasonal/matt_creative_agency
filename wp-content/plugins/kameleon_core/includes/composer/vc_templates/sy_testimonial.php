...
function sy_testmonial_shortC($atts,$content) {  
	//TESTMONIAL Attributes
 	extract(shortcode_atts(array(						
		...
		'testi_info_padding'	=>'',
		'icon_style'			=>'Defaults-km-icon-quote-',
 	...
 	$title_font = sy_composer_font_styles($test_title_font, $test_title_size, $test_title_style);
 	$content_font = sy_composer_font_styles($test_content_font, $test_content_size, $test_content_style);
 	$name_font = sy_composer_font_styles($test_name_font, $test_name_size, $test_name_style);


  	$img_src = wp_get_attachment_image_src($img,'kameleon-small-icon');
  	//Inside eStyle   	
  	...
  	if($style == "style1"){
  		...
  		$iconBorder	= sy_testimonial_borders('icon', $testi_icon_border, $icon_b_color);
  		//sy_testimonial_borders($type,$borders,$bWidth, $bColor)
		$pictureBorder	= sy_testimonial_borders('picture', $testi_picture_border, $picture_b_color);
		$infoBorder	= sy_testimonial_borders('info', $testi_info_border ,$info_b_color);
	...
	$content_style .= $contentPaddings .'"';
  	...
  	//Moved Things !!
	$picture = '<div class="km-testimonial-pic" style="'.$picturePaddings .'">'.$pictureBorder.'<img src="'.$img_src[0].'"></div>';		
  	$info = '<div class="km-testimonial-name-info" style="'.$infoPaddings.'" >'.$infoBorder.'<span class="km-testimonial-name" '.$name_style.'>'.$name.'</span><span class="km-testimonial-info" '.$title_style.'>'.$title.'</span></div>';
	...
	//Begin OutPut
	$outPut .= '<div class="km-testimonial" data-style="'.$style.'" data-pic-position="'.$position.'" style="background:'.$bg.'; '.$testi_container_margin.'"  '.$child_data.'>
					<div class="km-testimonial-content" '.$content_style.' >';
	...
	$outPut .= ($style == 'style3') ? '
	<div class="km-testimonial-content-icon" style="color:'.$tm_icon_color.';" data-position="left"><i class="'.$icon_style.'left"></i></div>
				<div class="km-testimonial-content-icon" style="color:'.$tm_icon_color.';" data-position="right"><i class="'.$icon_style.'right"></i></div>' 
				: '';
...
add_shortcode( 'sy_testmonial', 'sy_testmonial_shortC' );
...
function sy_testimonial_borders($type,$borders,$bColor){
	$result = '';
