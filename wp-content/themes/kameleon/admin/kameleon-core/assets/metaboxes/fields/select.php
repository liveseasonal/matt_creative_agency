...
* 
*/
class SayenThemes_Options_select{
	...
	function render(){		
		...
		$options_output = '';
		$options_output .= '<option value="'.esc_attr($value).'">'.esc_attr($options[$value]).'</option>';

		foreach ($options as $key => $val) {
			$options_output .= '<option value="'.esc_attr($key).'">'.esc_attr($val).'</option>';
		}
		...
		return '<div class="kmb-elem-attr" '.$dataRequired.'>
				...
				<div class="kmb-elem-attr-input"><select class="kmb-row-back-type full-select" name="'.esc_attr($name).'" >'.
						$options_output
					.'</select>
				</div>
			</div>';
